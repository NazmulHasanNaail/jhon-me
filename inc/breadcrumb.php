<?php
    ...
    if ( ! function_exists( 'mj_wp_breadcrumb' ) ) {
        function mj_wp_breadcrumb ( $list_style = 'ol', $list_id = 'breadcrumb', $list_class = 'breadcrumb', $list_li_class = 'breadcrumb-item', $active_class = 'active', $echo = true ) {
            ...
            // Front page
            if ( is_front_page() ) {
                $breadcrumb .= '<li class="'.$list_li_class.' '. $active_class . '">' . get_bloginfo( 'name' ). '</li>';
            } else {
                $breadcrumb .= '<li class="'.$list_li_class.'"><a href="' . home_url() . '">' . get_bloginfo( 'name' ) . '</a></li>';
            }
            ...
            if ( 'page' == get_option( 'show_on_front' ) && get_option( 'page_for_posts' ) ) {
                ...
                if ( is_home() ) {
                    $breadcrumb .= '<li class="'.$list_li_class.' ' . $active_class . '">' . get_the_title( $blog_page_id )  . '</li>';
                } else if ( is_category() || is_tag() || is_author() || is_date() || is_singular( 'post' ) ) {
                    $breadcrumb .= '<li class="'.$list_li_class.'"><a href="' . get_permalink( $blog_page_id ) . '">' . get_the_title( $blog_page_id )  . '</a></li>';
                }
            }

            // Category, tag, author and date archives
            if ( is_archive() && ! is_tax() && ! is_post_type_archive() ) {
                $breadcrumb .= '<li class="'.$list_li_class.' ' . $active_class . '">';
            ...
            if ( is_singular( 'post' ) ) {
                ...
                if ( $post_cats[0] ) {
                    $breadcrumb .= '<li class="'.$list_li_class.'"><a href="' . get_category_link( $post_cats[0]->term_id ) . '">' . $post_cats[0]->name . '</a></li>';
                }

                // Post title
                $breadcrumb .= '<li class="'.$list_li_class.' ' . $active_class . '">' . get_the_title() . '</li>';
            }
            ...
            if ( is_attachment() ) {
                ...
                if ( $post->post_parent ) {
                    $breadcrumb .= '<li class="'.$list_li_class.'"><a href="' . get_permalink( $post->post_parent ) . '">' . get_the_title( $post->post_parent ) . '</a></li>';
                }
            ...
            // Search
            if ( is_search() ) {
                $breadcrumb .= '<li class="'.$list_li_class.' ' . $active_class . '">' . __( 'Search', $text_domain ) . '</li>';
            }

            // 404
            if ( is_404() ) {
                $breadcrumb .= '<li class="'.$list_li_class.' ' . $active_class . '">' . __( '404', $text_domain ) . '</li>';
            }

            // Custom Post Type Archive
			if ( is_post_type_archive() ) {
				$breadcrumb .= '<li class="'.$list_li_class.' ' . $active_class . '">' . post_type_archive_title( '', false ) . '</li>';
			}
			...
			if ( is_tax() ) {
				...
				foreach ( $attached_to as $post_type ) {
					...
					if ( ! $output && get_post_type_archive_link( $cpt_obj->name ) ) {
						$breadcrumb .= '<li class="'.$list_li_class.'"><a href="' . get_post_type_archive_link( $cpt_obj->name ) . '">' . $cpt_obj->labels->name . '</a></li>';
						$output = true;
					}
				}

				// Term title
				$breadcrumb .= '<li class="'.$list_li_class.' ' . $active_class . '">' . single_term_title( '', false ) . '</li>';
			}
			...
			if ( is_single() && get_post_type() != 'post' && get_post_type() != 'attachment' ) {
				...
				if ( is_post_type_hierarchical( $cpt_obj->name ) ) {
					...
					if ( $post->post_parent ) {
						...
						foreach ( $crumbs as $crumb ) {
							$breadcrumb .= '<li class="'.$list_li_class.'">' . $crumb . '</li>';
						}
					}
				} else {
					// Like posts
					...
					if ( $cpt_taxes && is_taxonomy_hierarchical( $cpt_taxes[0] ) ) {
						...
						if ( is_array( $cpt_terms ) ) {
							...
							foreach( $cpt_terms as $cpt_term ) {
								if ( ! $output ) {
									$breadcrumb .= '<li  class="'.$list_li_class.'"><a href="' . get_term_link( $cpt_term->name, $cpt_taxes[0] ) . '">' . $cpt_term->name . '</a></li>';
									$output = true;
								}
							}
						}
